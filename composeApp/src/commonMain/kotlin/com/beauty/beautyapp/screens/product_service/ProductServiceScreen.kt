...
import com.beauty.beautyapp.screens.pos.search.SearchDialogItem
...
fun ProductServiceContent(viewModel: ProductServiceViewModel) {
    ...
    val filteredBeautyItems = remember { mutableStateOf(availableItems) }
    val searchText = remember { mutableStateOf("") }
    val keyboardController = LocalSoftwareKeyboardController.current

    LaunchedEffect(availableItems) {
        filteredBeautyItems.value = availableItems
    }
    ...
    if (state.value.isLoading) {
        ...
        Column(
            ...
            LazyColumn(
                ...
                items(filteredBeautyItems.value) {
    ...
    if (state.value.isDialogDisplayed) {
        ProductDialogScreen(
            beautyItem = state.value.selectedItem,
